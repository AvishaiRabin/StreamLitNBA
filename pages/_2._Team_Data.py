...
st.sidebar.markdown("# Team Data :handshake:")
...
if metric == "Standings":
    ...
    with col1:
        west_standings = standings[standings["Conference"] == "West"].reset_index(
            drop=True
        )
        west_standings.index = range(1, len(west_standings) + 1)
        st.dataframe(
            west_standings[["TeamName", "Record"]].style.apply(
                highlight_first_row, axis=1
            )
        )

    with col2:
        east_standings = standings[standings["Conference"] == "East"].reset_index(
            drop=True
        )
