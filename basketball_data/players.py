...
def get_player_career_stats(player_id: int):
    ...
    career_stats_df = career_stats_df.drop(['LEAGUE_ID', 'GS'], axis=1).groupby('SEASON_ID').agg(agg_dict)
...
@st.cache_data
def get_season_stats(season_list: list, stat: str, per_mode: str = 'PerGame', season_type: str = 'Regular Season'):
    """
    ...
    """
    ppg_df = pd.DataFrame(columns=['Season', '10th Percentile', '25th Percentile', '50th Percentile', '75th percentile',
                                   '90th percentile', 'Best in the League'])
