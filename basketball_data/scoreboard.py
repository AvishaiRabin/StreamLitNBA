...
teams_map = {
    teams_tbl["id"][i]: teams_tbl["nickname"][i] for i in range(len(teams_tbl))
}
...
def get_today_scoreboard():
    ...
    for game in games:
        ...
        todays_scoreboard = pd.concat(
            [todays_scoreboard, pd.DataFrame(this_game, index=[0])], ignore_index=True
        )
